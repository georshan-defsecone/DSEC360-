...
SELECT '4.3 Ensure ''CHECK_POLICY'' Option is set to ''ON'' for All SQL Authenticated Logins (Automated)', (
    SELECT * FROM (
        SELECT name, is_disabled as INT FROM sys.sql_logins WHERE is_policy_checked = 0
    ) AS src FOR JSON PATH, INCLUDE_NULL_VALUES
);
...
SELECT '7.3 Ensure Database Backups are Encrypted (Automated)', (
    SELECT * FROM (
        SELECT b.backup_set_id, b.database_name, b.server_name FROM msdb.dbo.backupset b inner join sys.databases d on b.database_name = d.name where b.key_algorithm IS NULL AND b.encryptor_type IS NULL AND d.is_encrypted = 0
    ) AS src FOR JSON PATH, INCLUDE_NULL_VALUES
);
