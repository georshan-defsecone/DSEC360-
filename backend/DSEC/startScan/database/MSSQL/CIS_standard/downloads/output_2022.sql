...
SELECT '2.11 Ensure SQL Server is configured to use non-standard ports (Automated)', (
    SELECT * FROM (
        SELECT CASE WHEN (SELECT value_data FROM sys.dm_server_registry WHERE value_name = 'ListenOnAllIPs') = 1 THEN (SELECT COUNT(*) AS Count FROM sys.dm_server_registry WHERE registry_key = '%IPAll%' AND value_name LIKE '%Tcp%' AND value_data = '1433') ELSE (SELECT COUNT(*) AS Count FROM sys.dm_server_registry WHERE value_name LIKE '%Tcp%' AND value_data = '1433') END AS Port1433Count
    ) AS src FOR JSON PATH, INCLUDE_NULL_VALUES
);
...
SELECT '3.13 Ensure membership in admin roles in MSDB database is limited (Automated)', (
    SELECT * FROM (
        SELECT COUNT(*) AS Count FROM sys.database_role_members AS drm INNER JOIN sys.database_principals AS r ON drm.role_principal_id = r.principal_id INNER JOIN sys.database_principals AS m ON drm.member_principal_id = m.principal_id WHERE r.name in ('db_owner, db_securityadmin, db_ddladmin, db_datawriter') and m.name <>'dbo'
    ) AS src FOR JSON PATH, INCLUDE_NULL_VALUES
);
