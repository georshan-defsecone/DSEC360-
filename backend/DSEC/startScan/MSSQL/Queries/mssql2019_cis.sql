-- Generated SQL script with nested JSON objects
...
SELECT '1.1 Ensure Latest SQL Server Cumulative and Security Updates are Installed (Manual)', (
    SELECT * FROM (
        SELECT SERVERPROPERTY('ProductLevel') as SP_installed, SERVERPROPERTY('ProductVersion') as Version, SERVERPROPERTY('ProductUpdateLevel') as 'ProductUpdate_Level',  SERVERPROPERTY('ProductUpdateReference') as 'KB_Number'
...
SELECT '2.11 Ensure SQL Server is configured to use non-standard ports (Automated)', (
    SELECT * FROM (
        SELECT CASE WHEN (SELECT value_data FROM sys.dm_server_registry WHERE value_name = 'ListenOnAllIPs') = 1 THEN (SELECT COUNT(*) AS Count FROM sys.dm_server_registry WHERE registry_key LIKE '%IPAll%' AND value_name LIKE '%Tcp%' AND value_data = '1433') ELSE (SELECT COUNT(*) AS Count FROM sys.dm_server_registry WHERE value_name LIKE '%Tcp%' AND value_data = '1433') END AS Port1433Count
    ) AS src FOR JSON PATH, INCLUDE_NULL_VALUES
);
...
INSERT INTO @results (Name, Result)
SELECT '2.14 Ensure the ''sa'' Login Account has been renamed (Automated)', (
    SELECT * FROM (
...
INSERT INTO @results (Name, Result)
SELECT '2.15 Ensure ''AUTO_CLOSE'' is set to ''OFF'' on contained databases (Automated)', (
    SELECT * FROM (
...
INSERT INTO @results (Name, Result)
SELECT '2.16 Ensure no login exists with the name ''sa'' (Automated)', (
    SELECT * FROM (
...
INSERT INTO @results (Name, Result)
SELECT '2.17 Ensure ''clr strict security'' Server Configuration Option is set to ''1'' (Automated)', (
    SELECT * FROM (
...
INSERT INTO @results (Name, Result)
SELECT '3.9 Ensure Windows BUILTIN groups are not SQL Logins (Automated)', (
    SELECT * FROM (
...
INSERT INTO @results (Name, Result)
SELECT '3.12 Ensure the ''SYSADMIN'' Role is Limited to Administrative or Built-in Accounts (Manual)', (
    SELECT * FROM (
        SELECT distinct(name),type_desc FROM master.sys.server_principals WHERE IS_SRVROLEMEMBER ('sysadmin',name) = 1 AND name not in ( 'NT SERVICE\SQLWriter', 'NT SERVICE\Winmgmt', 'NT SERVICE\MSSQLSERVER', 'NT SERVICE\SQLSERVERAGENT' )
    ) AS src FOR JSON PATH, INCLUDE_NULL_VALUES
);

INSERT INTO @results (Name, Result)
SELECT '4.2 Ensure ''CHECK_EXPIRATION'' Option is set to ''ON'' for All SQL Authenticated Logins Within the Sysadmin Role (Automated)', (
    SELECT * FROM (
...
INSERT INTO @results (Name, Result)
SELECT '5.3 Ensure ''Login Auditing'' is set to ''failed logins'' (Automated)', (
    SELECT * FROM (
        SELECT CASE @AuditLevel WHEN 0 THEN 'None' WHEN 1 THEN 'Success' WHEN 2 THEN 'Failure' WHEN 3 THEN 'Success and Failure' ELSE 'Unknown or Not Set' END AS AuditLevel
    ) AS src FOR JSON PATH, INCLUDE_NULL_VALUES
);
