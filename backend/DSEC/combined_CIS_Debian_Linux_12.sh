...
run_a__2_3_1_1() {
    ...
    jq -n --arg audit_id "2.3.1.1" --arg audit_name "Ensure a single time synchronization daemon is in use" --arg status "$status" --arg output "$output" \
        '{"audit_id": $audit_id, "audit_name": $audit_name, "status": $status, "output": $output}' >> tmp_results.json
    declare -A executed_dependent_audits
    echo "DEBUG: Parent audit 2.3.1.1 status is: $status"
    matched_case=false
    if echo "$output" | grep -Eq "systemd-timesyncd.service"; then
        run_a__2_3_2_1
        executed_dependent_audits["2.3.2.1"]=1
        run_a__2_3_2_2
        executed_dependent_audits["2.3.2.2"]=1
        matched_case=true
    fi
    if echo "$output" | grep -Eq "chrony.service"; then
        run_a__2_3_3_1
        executed_dependent_audits["2.3.3.1"]=1
        run_a__2_3_3_2
        executed_dependent_audits["2.3.3.2"]=1
        run_a__2_3_3_3
        executed_dependent_audits["2.3.3.3"]=1
        matched_case=true
    fi
    echo "DEBUG: Entering conditional block for 2.3.1.1. Current status: $status"
    if [[ "$status" = "FAIL" ]] || [[ "$status" = "ERROR" ]]; then
        echo "DEBUG: Inside FAIL/ERROR conditional for 2.3.1.1. Status being evaluated: $status"
        if [[ -z "${executed_dependent_audits["2.3.2.1"]}" ]]; then
        ...
        else # A case matched when parent passed, now skip others that were not executed
            if [[ -z "${executed_dependent_audits["2.3.2.1"]}" ]]; then
                jq -n --arg audit_id "2.3.2.1" --arg audit_name "Ensure systemd-timesyncd configured with authorized timeserver" \
                    --arg status "PASS" --arg output "Skipped: not applicable as a different firewall was detected by 2.3.1.1" \
                    '{"audit_id": $audit_id, "audit_name": $audit_name, "status": $status, "output": $output}' >> tmp_results.json
            fi
            if [[ -z "${executed_dependent_audits["2.3.2.2"]}" ]]; then
                jq -n --arg audit_id "2.3.2.2" --arg audit_name "Ensure systemd-timesyncd is enabled and running" \
...
run_a__4_1_1() {
    ...
    jq -n --arg audit_id "4.1.1" --arg audit_name "Ensure a single firewall configuration utility is in use" --arg status "$status" --arg output "$output" \
        '{"audit_id": $audit_id, "audit_name": $audit_name, "status": $status, "output": $output}' >> tmp_results.json
    declare -A executed_dependent_audits
    echo "DEBUG: Parent audit 4.1.1 status is: $status"
    matched_case=false
    if echo "$output" | grep -Eq "A single firewall is in use: ufw"; then
        run_a__4_2_1
        executed_dependent_audits["4.2.1"]=1
        run_a__4_2_2
        executed_dependent_audits["4.2.2"]=1
        run_a__4_2_3
        executed_dependent_audits["4.2.3"]=1
        run_a__4_2_4
        executed_dependent_audits["4.2.4"]=1
        run_a__4_2_6
        executed_dependent_audits["4.2.6"]=1
        run_a__4_2_7
        executed_dependent_audits["4.2.7"]=1
        matched_case=true
    fi
    if echo "$output" | grep -Eq "A single firewall is in use: nftables"; then
        run_a__4_3_1
        executed_dependent_audits["4.3.1"]=1
        run_a__4_3_2
        executed_dependent_audits["4.3.2"]=1
        run_a__4_3_3
        executed_dependent_audits["4.3.3"]=1
        run_a__4_3_4
        executed_dependent_audits["4.3.4"]=1
        run_a__4_3_5
        executed_dependent_audits["4.3.5"]=1
        run_a__4_3_5
        executed_dependent_audits["4.3.5"]=1
        run_a__4_3_6
        executed_dependent_audits["4.3.6"]=1
        run_a__4_3_8
        executed_dependent_audits["4.3.8"]=1
        run_a__4_3_9
        executed_dependent_audits["4.3.9"]=1
        matched_case=true
    fi
    if echo "$output" | grep -Eq "A single firewall is in use: iptables"; then
        run_a__4_4_1_1
        executed_dependent_audits["4.4.1.1"]=1
        run_a__4_4_1_2
        executed_dependent_audits["4.4.1.2"]=1
        run_a__4_4_1_3
        executed_dependent_audits["4.4.1.3"]=1
        run_a__4_4_2_1
        executed_dependent_audits["4.4.2.1"]=1
        run_a__4_4_2_2
        executed_dependent_audits["4.4.2.2"]=1
        run_a__4_4_3_1
        executed_dependent_audits["4.4.3.1"]=1
        run_a__4_4_3_2
        executed_dependent_audits["4.4.3.2"]=1
        matched_case=true
    fi
    echo "DEBUG: Entering conditional block for 4.1.1. Current status: $status"
    if [[ "$status" = "FAIL" ]] || [[ "$status" = "ERROR" ]]; then
        echo "DEBUG: Inside FAIL/ERROR conditional for 4.1.1. Status being evaluated: $status"
        if [[ -z "${executed_dependent_audits["4.2.1"]}" ]]; then
        ...
        if [[ "$matched_case" = false ]]; then
            ...
            if [[ -z "${executed_dependent_audits["4.2.1"]}" ]]; then
                jq -n --arg audit_id "4.2.1" --arg audit_name "Ensure ufw is installed" \
                    --arg status "PASS" --arg output "Skipped: not applicable as a different firewall was detected by 4.1.1" \
                    '{"audit_id": $audit_id, "audit_name": $audit_name, "status": $status, "output": $output}' >> tmp_results.json
            fi
            if [[ -z "${executed_dependent_audits["4.2.2"]}" ]]; then
                jq -n --arg audit_id "4.2.2" --arg audit_name "Ensure iptables-persistent is not installed with ufw" \
...
run_a__6_1_1_4() {
    ...
    jq -n --arg audit_id "6.1.1.4" --arg audit_name "Ensure only one logging system is in use" --arg status "$status" --arg output "$output" \
        '{"audit_id": $audit_id, "audit_name": $audit_name, "status": $status, "output": $output}' >> tmp_results.json
    declare -A executed_dependent_audits
    echo "DEBUG: Parent audit 6.1.1.4 status is: $status"
    matched_case=false
    if echo "$output" | grep -Eq "rsyslog"; then
        run_a__6_1_3_1
        executed_dependent_audits["6.1.3.1"]=1
        run_a__6_1_3_2
        executed_dependent_audits["6.1.3.2"]=1
        run_a__6_1_3_3
        executed_dependent_audits["6.1.3.3"]=1
        run_a__6_1_3_4
        executed_dependent_audits["6.1.3.4"]=1
        run_a__6_1_3_7
        executed_dependent_audits["6.1.3.7"]=1
        matched_case=true
    fi
    if echo "$output" | grep -Eq "journald"; then
        run_a__6_1_2_1_1
        executed_dependent_audits["6.1.2.1.1"]=1
        run_a__6_1_2_1_3
        executed_dependent_audits["6.1.2.1.3"]=1
        run_a__6_1_2_1_4
        executed_dependent_audits["6.1.2.1.4"]=1
        run_a__6_1_2_2
        executed_dependent_audits["6.1.2.2"]=1
        run_a__6_1_2_3
        executed_dependent_audits["6.1.2.3"]=1
        run_a__6_1_2_4
        executed_dependent_audits["6.1.2.4"]=1
        matched_case=true
    fi
    echo "DEBUG: Entering conditional block for 6.1.1.4. Current status: $status"
    if [[ "$status" = "FAIL" ]] || [[ "$status" = "ERROR" ]]; then
        echo "DEBUG: Inside FAIL/ERROR conditional for 6.1.1.4. Status being evaluated: $status"
        if [[ -z "${executed_dependent_audits["6.1.2.1.1"]}" ]]; then
        ...
        if [[ "$matched_case" = false ]]; then
            ...
            if [[ -z "${executed_dependent_audits["6.1.2.1.1"]}" ]]; then
                jq -n --arg audit_id "6.1.2.1.1" --arg audit_name "Ensure systemd-journal-remote is installed" \
                    --arg status "PASS" --arg output "Skipped: not applicable as a different firewall was detected by 6.1.1.4" \
                    '{"audit_id": $audit_id, "audit_name": $audit_name, "status": $status, "output": $output}' >> tmp_results.json
            fi
            if [[ -z "${executed_dependent_audits["6.1.2.1.3"]}" ]]; then
                jq -n --arg audit_id "6.1.2.1.3" --arg audit_name "Ensure systemd-journal-upload is enabled and active" \
...
run_a__6_2_1_1() {
    ...
    jq -n --arg audit_id "6.2.1.1" --arg audit_name "Ensure auditd packages are installed" --arg status "$status" --arg output "$output" \
        '{"audit_id": $audit_id, "audit_name": $audit_name, "status": $status, "output": $output}' >> tmp_results.json
    declare -A executed_dependent_audits
    echo "DEBUG: Parent audit 6.2.1.1 status is: $status"
    matched_case=false
    if echo "$output" | grep -Eq "All required audit packages are installed"; then
        run_a__6_2_1_2
        executed_dependent_audits["6.2.1.2"]=1
        run_a__6_2_1_3
        executed_dependent_audits["6.2.1.3"]=1
        run_a__6_2_1_4
        executed_dependent_audits["6.2.1.4"]=1
        run_a__6_2_2_1
        executed_dependent_audits["6.2.2.1"]=1
        run_a__6_2_2_2
        executed_dependent_audits["6.2.2.2"]=1
        run_a__6_2_2_3
        executed_dependent_audits["6.2.2.3"]=1
        run_a__6_2_2_4
        executed_dependent_audits["6.2.2.4"]=1
        run_a__6_2_3_1
        executed_dependent_audits["6.2.3.1"]=1
        run_a__6_2_3_2
        executed_dependent_audits["6.2.3.2"]=1
        run_a__6_2_3_3
        executed_dependent_audits["6.2.3.3"]=1
        run_a__6_2_3_4
        executed_dependent_audits["6.2.3.4"]=1
        run_a__6_2_3_5
        executed_dependent_audits["6.2.3.5"]=1
        run_a__6_2_3_6
        executed_dependent_audits["6.2.3.6"]=1
        run_a__6_2_3_7
        executed_dependent_audits["6.2.3.7"]=1
        run_a__6_2_3_8
        executed_dependent_audits["6.2.3.8"]=1
        run_a__6_2_3_9
        executed_dependent_audits["6.2.3.9"]=1
        run_a__6_2_3_10
        executed_dependent_audits["6.2.3.10"]=1
        run_a__6_2_3_11
        executed_dependent_audits["6.2.3.11"]=1
        run_a__6_2_3_12
        executed_dependent_audits["6.2.3.12"]=1
        run_a__6_2_3_13
        executed_dependent_audits["6.2.3.13"]=1
        run_a__6_2_3_14
        executed_dependent_audits["6.2.3.14"]=1
        run_a__6_2_3_15
        executed_dependent_audits["6.2.3.15"]=1
        run_a__6_2_3_16
        executed_dependent_audits["6.2.3.16"]=1
        run_a__6_2_3_17
        executed_dependent_audits["6.2.3.17"]=1
        run_a__6_2_3_18
        executed_dependent_audits["6.2.3.18"]=1
        run_a__6_2_3_19
        executed_dependent_audits["6.2.3.19"]=1
        run_a__6_2_3_20
        executed_dependent_audits["6.2.3.20"]=1
        run_a__6_2_4_1
        executed_dependent_audits["6.2.4.1"]=1
        run_a__6_2_4_2
        executed_dependent_audits["6.2.4.2"]=1
        run_a__6_2_4_3
        executed_dependent_audits["6.2.4.3"]=1
        run_a__6_2_4_4
        executed_dependent_audits["6.2.4.4"]=1
        run_a__6_2_4_5
        executed_dependent_audits["6.2.4.5"]=1
        run_a__6_2_4_6
        executed_dependent_audits["6.2.4.6"]=1
        run_a__6_2_4_7
        executed_dependent_audits["6.2.4.7"]=1
        run_a__6_2_4_8
        executed_dependent_audits["6.2.4.8"]=1
        run_a__6_2_4_9
        executed_dependent_audits["6.2.4.9"]=1
        run_a__6_2_4_10
        executed_dependent_audits["6.2.4.10"]=1
        matched_case=true
    fi
    echo "DEBUG: Entering conditional block for 6.2.1.1. Current status: $status"
    if [[ "$status" = "FAIL" ]] || [[ "$status" = "ERROR" ]]; then
        echo "DEBUG: Inside FAIL/ERROR conditional for 6.2.1.1. Status being evaluated: $status"
        if [[ -z "${executed_dependent_audits["6.2.1.2"]}" ]]; then
        ...
        if [[ -z "${executed_dependent_audits["6.2.3.16"]}" ]]; then
        ...
        if [[ -z "${executed_dependent_audits["6.2.3.17"]}" ]]; then
        ...
        if [[ -z "${executed_dependent_audits["6.2.3.18"]}" ]]; then
        ...
        if [[ "$matched_case" = false ]]; then
            ...
            if [[ -z "${executed_dependent_audits["6.2.1.2"]}" ]]; then
                jq -n --arg audit_id "6.2.1.2" --arg audit_name "Ensure auditd service is enabled and active" \
                    --arg status "FAIL" --arg output "Skipped: not applicable as a different firewall was detected by 6.2.1.1" \
                    '{"audit_id": $audit_id, "audit_name": $audit_name, "status": $status, "output": $output}' >> tmp_results.json
            fi
            if [[ -z "${executed_dependent_audits["6.2.1.3"]}" ]]; then
                jq -n --arg audit_id "6.2.1.3" --arg audit_name "Ensure auditing for processes that start prior to auditd is enabled" \
...
run_a__6_3_1() {
    ...
    jq -n --arg audit_id "6.3.1" --arg audit_name "Ensure AIDE is installed" --arg status "$status" --arg output "$output" \
        '{"audit_id": $audit_id, "audit_name": $audit_name, "status": $status, "output": $output}' >> tmp_results.json
    declare -A executed_dependent_audits
    echo "DEBUG: Parent audit 6.3.1 status is: $status"
    matched_case=false
    if echo "$output" | grep -Eq "aide is installed"; then
        run_a__6_3_2
        executed_dependent_audits["6.3.2"]=1
        run_a__6_3_3
        executed_dependent_audits["6.3.3"]=1
        matched_case=true
    fi
    echo "DEBUG: Entering conditional block for 6.3.1. Current status: $status"
    if [[ "$status" = "FAIL" ]] || [[ "$status" = "ERROR" ]]; then
        echo "DEBUG: Inside FAIL/ERROR conditional for 6.3.1. Status being evaluated: $status"
        if [[ -z "${executed_dependent_audits["6.3.2"]}" ]]; then
        ...
        if [[ "$matched_case" = false ]]; then
            ...
            if [[ -z "${executed_dependent_audits["6.3.2"]}" ]]; then
                jq -n --arg audit_id "6.3.2" --arg audit_name "Ensure filesystem integrity is regularly checked" \
                    --arg status "FAIL" --arg output "Skipped: not applicable as a different firewall was detected by 6.3.1" \
                    '{"audit_id": $audit_id, "audit_name": $audit_name, "status": $status, "output": $output}' >> tmp_results.json
            fi
            if [[ -z "${executed_dependent_audits["6.3.3"]}" ]]; then
                jq -n --arg audit_id "6.3.3" --arg audit_name "Ensure cryptographic mechanisms are used to protect the integrity of audit tools" \
...
# Finalize JSON
jq -s 'sort_by(.audit_id | split(".") | map(try tonumber catch .))' tmp_results.json > /tmp/results_CIS_Debian_Linux_12.json
rm -f tmp_results.json
echo "Audit complete. Results saved to results_CIS_Debian_Linux_12.json"
